...
| WeChat | `dsh plugin --profile web add dsh-im dsh-im-weixin` | **Web QR bind (recommended)**: Settings → Plugins → WeChat → "Scan to bind", then scan with WeChat (pairing digits entered in the page); prerequisite: your account has the WeChat bot entry under Me → Settings → Plugins ([guide](docs/weixin-setup.md)) | Restart `dsh web` after binding, then message the bot in WeChat: `/new` → send tasks; approvals come as text — reply `/approve <id> yes` (no buttons) |

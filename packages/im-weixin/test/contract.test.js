...
test('契约缺失必须抛错（FR-9.4 不兼容即报错）', () => {
  assert.throws(() => validateAdapterContract({ platform: 'weixin' }));
  assert.throws(() => validateAdapterContract({ send: async () => ({}) }));
  assert.throws(() => validateAdapterContract({ platform: 'weixin', send: 'not-a-fn' }));
});

test('缺少 lastEventAt 心跳 → warnings（可观测性提示）', () => {
  const r = validateAdapterContract({ platform: 'weixin', status: { connected: false }, send: async () => ({}) });
  assert.equal(r.ok, true);

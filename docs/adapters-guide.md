...
- 出站可带 `title`（卡片标题）：审批卡片默认「🔐 审批请求」，信任卡片为「🔐 信任确认」；适配器渲染 header 时用 `out.title`

## 七·一、流式消息原地更新（edit 契约，对照成熟 Claude Code↔IM 桥的打字机体验）

核心的流式增量（`notifications.streamEdit: true`，默认开）遵循「首帧发送 + 后续帧原地编辑」：

1. 核心发首帧：`out = { text, stream: true, title }` → 渠道 **send() 必须返回真实 `messageId`**
2. 后续帧：核心调用渠道 **`edit(messageId, out)`** 原地更新同一条消息
3. `edit()` 抛错 → 核心自动回退「发新消息」，**绝不丢内容**；渠道不实现 edit 或 send 不返回
   messageId → 核心自动退化为逐条新消息

适配器实现要点（飞书为参考实现）：

- 平台「编辑已发送消息」的 API 往往只支持特定消息类型（飞书 `im.message.patch`
  只能改 interactive 卡片，文本消息不可编辑）→ **流式帧一律渲染为卡片**
- 卡片单帧正文有字段长度限制（飞书 `lark_md` 取保守 2800 字符）：核心已做尾部截断，
  适配器再兜底一次（`slice(-2800)`）
- 业务失败（`code != 0`）必须 throw 且带平台错误码——核心依赖这个异常来触发回退
...
□ 连接方式决策表（免公网优先）
□ 可观测三件套：状态 + 最近事件心跳 + 边界日志（带平台错误码；出站业务失败必须 throw）
□ ID 类型学：入站学习 + 前缀启发
□ parseContent 纯函数 + 单测
□ 按钮 → 平台原生组件 + 回调 → 中性 data；卡片标题用 out.title
□ 流式 edit：send 返回 messageId + edit() 原地更新（平台 patch 只支持卡片 → 流式帧渲染为卡片）；
  核心不实现时的降级路径 = 逐条新消息

...
## ① 接入飞书（二选一：扫码 1 分钟 / 手动 10 分钟）

### 方式一：扫码接入（推荐，约 1 分钟）

不需要手动建应用、勾权限、配订阅——**飞书扫码一键完成**，应用名、权限、事件、回调都按本插件需求**预填**：

1. 终端运行（需要已安装 dsh-im-feishu，见 ②）：
   ```sh
   npx -y dsh-im-feishu-qr
   ```
2. 终端出现**二维码** → 用**手机飞书扫码** → 在确认页直接确认（预填项都在，无需改动）
3. 看到「✅ 凭据已安全写入 …/feishu-credentials.json」即完成
4. 重启 `dsh web`，在飞书里私聊机器人开始使用

> 说明：
> - 凭据写入 `$DSH_HOME/dsh-im/feishu-credentials.json`（仅本机可读），**无需设置任何环境变量**；
>   若同时设置了 `FEISHU_APP_ID / FEISHU_APP_SECRET` 环境变量，环境变量优先。
> - 预填的权限/事件/回调走飞书官方灰度（平台支持才自动生效）；灰度未覆盖时确认页是默认模板，
>   按下方「方式二」手动补勾即可，结果一样。
> - 扫码创建的应用属于扫码人所在企业，需要是企业成员（自己当管理员最省事）。
> - 想创建多个机器人？重复扫码即可，每次新建一个应用；删掉凭据文件即解除接入。

### 方式二：手动创建（扫码失败 / 需要精细控制时）
